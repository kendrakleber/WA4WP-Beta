...
/**
 * Plugin Name: WA4WP
 * Plugin URI: https://www.newpathconsulting.com/wild-apricot-for-wordpres
 ...
 * Author URI: https://www.newpathconsulting.com/
 **/
...
$customourplugin = scandir($destination);
require_once ABSPATH . 'wp-admin/includes/plugin.php';
$tocheckarray = array("advanced-custom-fields", 'wild-apricot-login');
$inarraychecking = count(array_intersect($tocheckarray, $customourplugin));
$apl = get_option('active_plugins');
$plugins = get_plugins();
$activated_plugins = array();
foreach ($apl as $p) {
    if (isset($plugins[$p])) {
        array_push($activated_plugins, $plugins[$p]['TextDomain']);
    }
}
if ($inarraychecking < 2) {
    //  echo "coming";
    ...
    if ($res === true) {
        ...
        $zip->close();
        if (!in_array('acf', $activated_plugins)) {
            activate_plugin('advanced-custom-fields/acf.php');
        }
        // if (!in_array('wild-apricot-login', $activated_plugins)) {
        ...
        // {
        //     $add_plugin = 'ultimate-member/ultimate-member.php';
...
// for updating the role based restriction
    ...
    // if (function_exists('acf_add_options_page')) {
    //     $member_status = get_field("membershipstatus", "option");
    // }
...
// die;
    ...
    // foreach ($currentuserrole as $role => $newroles) {
    //     $result = substr($newroles, 0, 4);

    //     if ($result!='grp_') {

    //        $leve_role_userss[] =$newroles;
    //     }
    //     else
    //     {
    //         $grouparray_memberss[]=$newroles;
    //     }
    //    }
    ...
    // echo "</pre>";
        ...
        // if(!empty($grouparray_memberss))
        // {
        //     $groupcheckarray=array_intersect($grouparray_memberss,$rolegetdb);
        //     $group_count= count($groupcheckarray);
        //     if($group_count<=0)
        //     {
...
// //$expiration = 1*24*60*60; //UPDATE HERE;
// $expiration = 5*60; //UPDATE HERE;
// }
...
function do_this_hourly() {
    ...
    // echo  "coming inside the cron job";
    // die;
    $curl = curl_init();
    curl_setopt_array($curl, array(
        CURLOPT_URL => 'https://oauth.wildapricot.org/auth/token',
...
foreach($apiupdate_response as $responsear)
{
    if(isset($responsear[0]->MembershipLevel->Name))
    {
        ...
        // $roledescription = str_replace(" ", "", $newroleforuser);
        // $converted_addrole = strtolower($roledescription);
        // $new_str = preg_replace('~[,(+)-]~', '_', $converted_addrole);
